...
void	bkydel_all(HDL h_btr, bool del_rhdl);					// del ALL keys and maybe recs
...
void	*btrcargo(HDL btr,void *cargo);							// get/update 22-byte btr cargo
void	btrclose(HDL btr);
int32_t	btrnkeys(HDL btr);
void	btrrls(HDL btr);
void	btr_set_cmppart(HDL btr, PFI_v_v func);					// (db internal)
RHDL	btrist(HDL db, int keytyp, int keysiz);
HDL	btropen(HDL db,RHDL rhdl_btr);
...
void	dbckpt(HDL db);
void	dbsafeclose(HDL db);	// (in DBX) Close database AND unset db.hdr 'safe' flag if was set to 'dirty'
...
class DBX {    // base class for my databases to share filename() method - also DbClose() in destructor
protected:
	HDL db;     // DON'T define another one of these in derived DB classes! Fails with multiple DbClose()
public:
   ~DBX() {if (db) {dbsafeclose(db); db=NULL;}};
